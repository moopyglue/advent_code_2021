...
for a in range(len(bingocalls)):
    ...
    for b in range(len(cards)):
        ...
        if check_card(cards[b],bingocalls[0:a+1]):
            # found matching card caluclate reulst and print
            print("card=%d, sum_unmarked=%d, lastcalled=%s" 
                  % (b,sum_unmarked(cards[b],bingocalls[0:a+1]),bingocalls[a] ))
            print("result=%d" % 
                  (sum_unmarked(cards[b],bingocalls[0:a+1])*int(bingocalls[a])))
            exit(0)
